...
async function fetchAllDestinations(page) {
  try {
    ...
    let url = 'http://127.0.0.1:8080/testdata';
...
function createCard(item) {
  ...
  img.src = item.fields.product_image

  // img.setAttribute("src", `{% static ${item.fields.product_image} %}`);

  let content = document.createElement("div");
  content.innerHTML = `<i class="fa-solid fa-plane-departure"></i>`;

  let title = document.createElement("h4");
  title.innerText = item.fields.description;
  ...
  price.innerHTML = `<span>${item.fields.price}</span> per person`;
